...
- **`/command-center`** — Full-page 3D operational map (MapLibre GL) with 9 toggleable layer groups:
  ...
  - **PHIVOLCS Hazards** — Liquefaction + Tsunami inundation overlays sourced from PHIVOLCS HazardHunterPH (KMZ ground overlays served as raster image sources), with per-layer opacity slider
